...
if (isset($_GET['other'])) {
  $QUE    = $SURVEY->getQuestion(filter_input(INPUT_GET,'other',FILTER_VALIDATE_INT));
  $qdata  = '';
  // Load unique answers for this question..
  $query = mysqli_query($connect,"SELECT ans_text FROM ".DB_PREFIX."answers 
                        WHERE ans_que_id  = '".filter_input(INPUT_GET,'other',FILTER_VALIDATE_INT)."'
                        AND ans_var_id    = '0'       
                        ORDER BY ans_id
                        ") or die(db_MSG(__FILE__,__LINE__,$connect));
  while ($ANS = mysqli_fetch_object($query)) {
    if ($ANS->ans_text) {
      // Get count of other answers for this question...
      $tCount      = mysqli_num_rows($query);
      // How many times does this answer occur..
      $qCount      = mysqliTableCount('answers',' WHERE ans_text = \''.safeImport($ANS->ans_text).'\'');
