...
----------------------------------------------*/
// Catch MySQL errors..
function db_MSG($file,$line,$conn = "empty") {
  if ($conn == "empty") {
    $conn = mysqli_connect('re9','hans','hanshans','survey');
  }
  ...
  echo '<b>Code</b>: '.mysqli_errno($conn).'<br />';
...
function loadSettings() {
  global $database;
  $connect = mysqli_connect('re9','hans','hanshans','survey');
  $query = mysqli_query($connect,"SELECT * FROM ".DB_PREFIX."config LIMIT 1") or die(db_MSG(__FILE__,__LINE__,$connect));
  return mysqli_fetch_object($query);
}

// Table count..
function mysqliTableCount($table,$query='',$row='count(*) AS r_count',$num=false) {
  global $database;
  $connect = mysqli_connect('re9','hans','hanshans','survey');
  $count = mysqli_query($connect,"SELECT ".$row." FROM ".DB_PREFIX.$table.$query."") or die(db_MSG(__FILE__,__LINE__,$connect));
  if ($num) {
