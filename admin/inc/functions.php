...
function getLastQuestion($id) {
  ...
  $id    = (int)$id;
  $query = mysqli_query($connect,"SELECT que_id FROM ".DB_PREFIX."questions WHERE que_sur_id = '$id' ORDER BY que_id DESC LIMIT 1") or die(db_MSG(__FILE__,__LINE__,$connect));
...
function rowData($table,$query='') {
  global $database;
  $connect = mysqli_connect('re9','hans','hanshans','survey');
  $query = mysqli_query($connect,"SELECT * FROM ".DB_PREFIX.$table.$query."") or die(db_MSG(__FILE__,__LINE__,$connect));
