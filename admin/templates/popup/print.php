...
if (mysqli_num_rows($q_keys)>0) {
  while ($KEYS = mysqli_fetch_object($q_keys)) {
    $run = ++$count;
    echo '<td style="text-align:left;vertical-align:top;width:'.round(100/3).'%"><b>'.str_replace('.','',cleanDataEnt($KEYS->key_word)).'</b> <span style="font-size:11px;color:#666">('.number_format($KEYS->k_count).' - '.number_format($KEYS->k_count/mysqli_num_rows($q_keys)*100,2).'%)</span></td>'."\n";
    if ($run%3==0) {
     echo ($run!=mysqli_num_rows($q_keys) ? '</tr>'."\n" : '');
     echo ($run!=mysqli_num_rows($q_keys) ? '<tr>'."\n" : '');
    }
  }
}
