...
?>		
  ...
  //]]>
      ...
      <?php
      $q_questions = mysqli_query($connect,"SELECT * FROM ".DB_PREFIX."questions
                                  WHERE que_sur_id = '".(int)$_GET['survey']."'
                                  ORDER BY orderBy
                                  ") or die(db_MSG(__FILE__,__LINE__,$connect));
      
      while ($QUESTIONS = mysqli_fetch_object($q_questions)) {
      ?>
      ...
      <?php
      $q_keys_count = mysqli_query($connect,"SELECT DISTINCT(key_word) FROM ".DB_PREFIX."keywords
                                   ...
                                   ORDER BY key_word
                                   ") or die(db_MSG(__FILE__,__LINE__,$connect));
      if (mysqli_num_rows($q_keys_count)>0) {    
      $q_keys = mysqli_query($connect,"SELECT DISTINCT(key_word),key_word,key_que_id,key_sur_id,count(*) AS k_count FROM ".DB_PREFIX."keywords
                             WHERE key_sur_id = '".(int)$_GET['survey']."'
                             ".(isset($_GET['question']) && ctype_digit($_GET['question']) ? 'AND key_que_id = \''.(int)$_GET['question'].'\'' : '')."
                             ".(isset($_GET['from']) &&  $_GET['from'] && isset($_GET['to']) && $_GET['to'] ? 'AND key_date BETWEEN \''.$_GET['from'].'\' AND \''.$_GET['to'].'\'' : '')."
      ...
      if (mysqli_num_rows($q_keys)>0) {
        ...
        <?php
        while ($KEYS = mysqli_fetch_object($q_keys)) {
        ?>
        <div class="keyWrap">
        <table width="100%" cellspacing="0" cellpadding="0">
        <tr>
          ...
          <td style="width:15%;text-align:center"><?php echo number_format($KEYS->k_count/mysqli_num_rows($q_keys_count)*100,2); ?>%</td>
        </tr>
        </table>
        </div>
        <?php
        }
        echo page_numbers(mysqli_num_rows($q_keys_count),$limit,$page);
        ?>
        <p>&nbsp;</p>
        <?php
      } else {
