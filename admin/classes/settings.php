...
function updateSettings () {
  ...
  mysqli_query($connect,"UPDATE ".$this->prefix."config SET
...
function loadSettings () {
  $connect = mysqli_connect('re9','hans','hanshans','survey');
  $query = mysqli_query($connect,"SELECT * FROM ".$this->prefix."config LIMIT 1") or die(db_MSG(__FILE__,__LINE__,$connect));
...
function importFileCSV($name,$email,$seperator,$id,$msg) {
  ...
  $query = mysqli_query($connect,"SELECT * FROM ".$this->prefix."users
                        ".($id!='all' ? 
