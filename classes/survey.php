...
function checkSurveyExpiryDate() {
  ...
  return (date("Y-m-d") >= $row->sur_date_expire && $row->sur_date_expire!='1000-01-01' ? true : false);
}
...
function getParticipantCount($id) {
  $id    = (int)$id;
  $connect = mysqli_connect('re9','hans','hanshans','survey');
  $query = mysqli_query($connect,"SELECT DISTINCT(ans_session_id) FROM ".$this->prefix."answers 
                        WHERE ans_sur_id = '$id'
                        ") or die(db_MSG(__FILE__,__LINE__,$connect));
...
function loadSurveyQuestions($id) {
  ...
  $id    = (int)$id;
  $connect = mysqli_connect('re9','hans','hanshans','survey');
  $query = mysqli_query($connect,"SELECT que_id FROM ".$this->prefix."questions 
                        WHERE que_sur_id = '$id'
                        ORDER BY orderBy
                        ") or die(db_MSG(__FILE__,__LINE__,$connect));
...
function getSurveyQuestionCount($id) {
  $id     = (int)$id;
  $connect = mysqli_connect('re9','hans','hanshans','survey');
  $query = mysqli_query($connect,"SELECT count(*) AS q_count FROM ".$this->prefix."questions 
...
function loadOrderOfImportanceArray($id) {
  $id    = (int)$id;
  $connect = mysqli_connect('re9','hans','hanshans','survey');
  $query = mysqli_query($connect,"SELECT count(*) AS v_count FROM ".$this->prefix."variants WHERE var_que_id = '$id'") or die(db_MSG(__FILE__,__LINE__,$connect));
  $row   = mysqli_fetch_object($query);
...
function logUserData($id,$void=false) {
  ...
  $connect = mysqli_connect('re9','hans','hanshans','survey');
  mysqli_query($connect,"INSERT INTO ".$this->prefix."users (
               usr_sur_id,
               usr_email,
               usr_name,
               usr_date,
    ...
    if (isset($_SESSION['field_'.$_SESSION['session_que_id'][$i]]) && is_array($_SESSION['field_'.$_SESSION['session_que_id'][$i]])) {
      foreach ($_SESSION['field_'.$_SESSION['session_que_id'][$i]] AS $value) {
        ...
        if ($_SESSION['session_build_type'][$_SESSION['session_que_id'][$i]]==7) {
          mysqli_query($connect,"INSERT INTO ".$this->prefix."answers (
                       ans_sur_id,
                       ans_que_id,
                       ans_var_id,
                       ans_text,
    ...
    if ($field) {
      ...
      for ($j=0; $j<count($words); $j++) {
        ...
        if (!empty($common) && !in_array($words[$j],$common) && strlen($words[$j])>2) {
          mysqli_query($connect,"INSERT INTO ".$this->prefix."keywords (
                       key_sur_id,
                       key_que_id,
                       key_word,
                       key_date
