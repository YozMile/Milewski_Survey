  ...
  switch ($QUESTION->que_answer_type) {
    ...
    if (mysqli_num_rows($query)>0) {
      while ($row = mysqli_fetch_object($query)) {
        $string .= str_replace(array('{variant}','{percentage}','{b}','{bar}'),array($this->cleanData(trim($row->var_text)),$this->calculateAnswerPercentage($id,$row->var_opt_id),(++$count==mysqli_num_rows($query) && !in_array($QUESTION->que_answer_type,array(3,6)) ? 'b' : ''),$this->progressBar($this->calculateAnswerPercentage($id,$row->var_opt_id,false,true))),file_get_contents(PATH.'templates/html/result_type_options.tpl'));
      }
    }
    ...
    // Order of importance..
    case '7':
    $query = mysqli_query($connect,"SELECT ans_var_id,ans_que_id,COUNT(*) AS v_count,SUM(ans_text) AS s_count FROM ".$this->prefix."answers 
                          WHERE ans_que_id = '$id'
    ...
    if (mysqli_num_rows($query)>0) {
      while ($row = mysqli_fetch_object($query)) {
        ...
        $string      .= str_replace(array('{variant}','{percentage}','{b}','{bar}'),array($this->cleanData(trim($var->var_text)),$percentage,(++$count==mysqli_num_rows($query) && !in_array($QUESTION->que_answer_type,array(3,6)) ? 'b' : ''),$this->progressBar($raw)),file_get_contents(PATH.'templates/html/result_type_options.tpl'));
      }
    }
    break;
    ...
    case '8':
    case '9':
    $query = mysqli_query($connect,"SELECT * FROM ".$this->prefix."answers 
                          WHERE ans_que_id  = '$id'
                          AND ans_text     != ''
                          ") or die(db_MSG(__FILE__,__LINE__,$connect));
    if (mysqli_num_rows($query)>0) {
...
function calculateAnswerPercentage($question,$variant,$other=false,$raw=false) {
  ...
  $query = mysqli_query($connect,"SELECT count(*) AS a_count FROM ".$this->prefix."answers 
  ...
  $query2 = mysqli_query($connect,"SELECT count(*) AS v_count FROM ".$this->prefix."answers 
                         WHERE ans_que_id  = '$question' 
                         AND ans_var_id    = '$variant' 
                         AND ans_sur_id    = '".$sur->sur_id."'
                         ") or die(db_MSG(__FILE__,__LINE__,$connect));
  $count2 = mysqli_fetch_object($query2);
